...
resource "null_resource" "fetch_kubeconfig" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      ssh -o StrictHostKeyChecking=no -i ${pathexpand(var.ssh_private_key_path)} ubuntu@${aws_eip.k3s.public_ip} "sudo cat /var/lib/rancher/k3s/server/node-token" > ${var.node_token_output_path}
      chmod 600 ${var.node_token_output_path}
      echo "kubeconfig saved: ${var.kubeconfig_output_path} (context: ${var.context_name})"
      echo "node-token saved: ${var.node_token_output_path}"
    EOT
    interpreter = ["bash", "-c"]
  }
}
